...
module.exports = function(sequelize, DataTypes) {
  var Company = sequelize.define("Company", {
    ...
    oauth_auth_config : {
        ...
        set : function(val){
          if (! val || typeof val !== 'object'  ) {
            // Convert empty object to string representation
            // as this is how it is going to be stored in database
            val = JSON.stringify(val || {});
          } else {
            val = JSON.stringify(val);
          }
          this.setDataValue( 'oauth_auth_config', val );
        },

        get : function(){
          var val = this.getDataValue('oauth_auth_config');
          try {
            // defaults to empty object if there is no JSON in database
            val = JSON.parse(val || '{}');
          } catch(err) {
            console.error(
              'Failed to parse the OAuth settings saved in company %s. Error: %s',
              this.id, err
            );
            val = {};
          }

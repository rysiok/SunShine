...
get_deducted_days : function(args) {
  ...
  var leave_days = [],
    ...
    if(args && args.hasOwnProperty('startDate')) {
        startDate = moment.utc(args.startDate);
    }
    if(args && args.hasOwnProperty('endDate')) {
        endDate = moment.utc(args.endDate);
    }
  ...
  leave_days = _.filter(
    _.map(this.get_days(), function(leave_day){
      ...
      if (startDate && moment.utc(leave_day.date) < startDate) return;
      if (endDate && moment.utc(leave_day.date) > endDate) return;

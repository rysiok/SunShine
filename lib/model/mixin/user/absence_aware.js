...
module.exports = function(sequelize){
  ...
  this.promise_to_update_carried_over_allowance = function(args) {
    ...
    this.promise_to_update_allowance_adjustment = function(args) {
    let
      id = this.id,
    ...
    return sequelize.models.UserAllowanceAdjustment
      ...
      .then(allowance_adjustment => {
        if (allowance_adjustment) {
            let not_used = allowance_adjustment.carried_over_allowance - taken;
            if (not_used > 0) {
                allowance_adjustment.adjustment = allowance_adjustment.adjustment - not_used;
            }
            return allowance_adjustment.save();
        }
      });
  };

...
module.exports = function(passport) {
  ...
  // Route to initiate Azure AD OIDC authentication
  router.get('/login/azure',
    passport.authenticate('azuread-openidconnect', {
      failureRedirect: '/login',
      // Optionally, specify scopes if not in strategy default
      // scope: [ 'profile', 'email', 'openid' ] 
    })
  );

  // Route to handle the callback from Azure AD
  router.post('/auth/openid/return',
    passport.authenticate('azuread-openidconnect', {
      failureRedirect: '/login',
      // failureFlash: 'Failed to authenticate with Azure AD. Please try again or contact support.', // Optional: use connect-flash for messages
    }),
    function(req, res) {
      // Successful authentication
      req.session.flash_message('Welcome back ' + req.user.name + '!');

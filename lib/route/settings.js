...
router.get('/company/authentication/oauth', function(req, res){
  req.user
    .getCompany()
    .then(function(company){
      const azureAdConfig = config.get('azure_ad') || {};
      res.render('settings_company_authentication_oauth', {
        company: company,
        // oauth_auth_config is already an object due to the model's getter
        azure_ad_callback_url: company.application_domain + (azureAdConfig.callback_url || '/auth/openid/return'),
      });
    })
    .catch(function(error){
      console.error("Failed to load OAuth settings page: " + error);
      req.session.flash_error('Failed to load OAuth settings page. Please try again.');
      return res.redirect_with_session('/settings/general/');
    });
});

router.post('/company/authentication/oauth', function(req, res){
  const {
    oauth_auth_enabled,
    oauth_client_id,
    oauth_client_secret, // This might be empty if user doesn't want to change it
    oauth_tenant_id,
  } = req.body;

  const enabled = validator.toBoolean(oauth_auth_enabled || 'false');

  if (enabled) {
    if (!validator.trim(oauth_client_id)) {
      req.session.flash_error('Client ID cannot be empty when OAuth is enabled.');
    }
    if (!validator.trim(oauth_tenant_id)) {
      req.session.flash_error('Tenant ID cannot be empty when OAuth is enabled.');
    }
  }

  if (req.session.flash_has_errors()) {
    return res.redirect_with_session('/settings/company/authentication/oauth');
  }

  req.user.getCompany()
    .then(function(company){
      company.oauth_auth_enabled = enabled;

      const currentConfig = company.oauth_auth_config || {};
      let newClientSecret = currentConfig.client_secret; // Keep old secret by default

      // Only update client_secret if a new value is provided
      const trimmedClientSecret = validator.trim(oauth_client_secret);
      if (trimmedClientSecret) {
        newClientSecret = trimmedClientSecret;
      }

      // The company model's setter for oauth_auth_config will handle stringification
      company.oauth_auth_config = {
        client_id: validator.trim(oauth_client_id),
        client_secret: newClientSecret,
        tenant_id: validator.trim(oauth_tenant_id),
      };

      return company.save();
    })
    .then(function(){
      req.session.flash_message('OAuth configuration was successfully updated.');
      return res.redirect_with_session('/settings/company/authentication/oauth');
    })
    .catch(function(error){
      console.error("An error occurred while trying to update OAuth configuration: " + error);
      req.session.flash_error('Failed to update OAuth configuration. ' + (error.show_to_user ? error : 'Please contact customer service.'));
      return res.redirect_with_session('/settings/company/authentication/oauth');
    });
});

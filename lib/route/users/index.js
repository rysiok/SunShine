...
router.post('/edit/:user_id/', function(req, res){
  ...
  .then(function(company){
    ...
    new_user_attributes = get_and_validate_user_parameters({
      ...
      item_name   : 'user',
      departments : company.departments,
      employee    : company.users[0], // Pass the employee object
    });
...
function get_and_validate_user_parameters(args) {
    var req         = args.req,
        ...
        employee    = args.employee, // Optional employee object
    ...
    // Initialize auto_approve
    let auto_approve_val = false; // Default for new users

    if (employee && typeof employee.auto_approve !== 'undefined') {
        auto_approve_val = employee.auto_approve;
    }

    // If auto_approve is in request body, it takes precedence
    if (typeof req.body['auto_approve'] !== 'undefined' && req.body['auto_approve'] !== null) {
        auto_approve_val = validator.toBoolean(req.body['auto_approve']);
    }
